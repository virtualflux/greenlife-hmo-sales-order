...
const DrugsComponent: React.FC<IDrugComponent> = ({ form, customerType }) => {
  ...
  const [inventoryDrug, setInventoryDrug] = useState({
    name: "",
    item_id: "",
    locationQty: 0,
    unit: 0,
  });
  const [checkingAvailability, setCheckAvailabilty] = useState<
    boolean | undefined
  >(undefined);
  ...
  } = useQuery({ queryKey: ["inventory-items"], queryFn: getInventoryItems });
  ...
  const addDrug = () => {
    if (newDrug.price <= 0) {
      toast.warn("This item's price is not available.");
      return;
    }

    if (newDrug.name && newDrug.id && newDrug.quantity > 0) {
      form.setFieldValue("drugs", [...form.values.drugs, newDrug]);
      const description = form.values.description;
      form.setFieldValue(
        "description",
        description ? description + "," + newDrug.name : newDrug.name
      );
      setNewDrug({
        name: "",
        id: "",
  ...
  const handleInventoryItemSelect = async (value: {
    name: string;
    id: string;
  }) => {
    ...
    const res = await axios.get<{ item: Item }>(
      `/api/zoho/inventory-item/${value.id}`
    );
    ...
    setCheckAvailabilty(false);
    // console.log({ item })
    const pickedLocation = item.locations.find(
      (loc) => loc.location_id == form.values.location
    );
    ...
    setInventoryDrug((prev) => ({
      ...prev,
      name: item.name,
      item_id: item.item_id.toString(),
      unit: pickedLocation.initial_stock_rate,
      locationQty: parseFloat(pickedLocation.location_stock_on_hand),
    }));
    setNewDrug({
      ...newDrug,
      id: value.id,
      name: value.name,
      unit: pickedLocation.initial_stock_rate,
      price: parseFloat((unitPrice * (newDrug.quantity || 1)).toFixed(2)),
      quantity: newDrug.quantity ? newDrug.quantity : 1,
    });
  };
  ...
  const handleQuantityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setNewDrug((prev) => ({
      ...
      price: parseFloat((newDrug.unit * quantityVal).toFixed(2)),
    }));
  };
  ...
  return (
    <div className="border-t pt-4 w-full">
      ...
      <div className="mb-4 p-3 w-full bg-[#d3d7c9] rounded text-zinc-800">
        <h4 className="font-medium">Add New Drug</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div className="md:col-span-2">
            <label
              htmlFor="inventoryItemName"
              className="inline-flex text-sm font-medium w-full"
            >
            ...
            <span className="h-2 w-full">
              {checkingAvailability == true && (
                <p className="text-gray-500 italic text-xs">
                  ...
                  <span className="text-sm font-semibold loading-ellipses">
                    ...
                  </span>
                </p>
              )}
              {newDrug.id && checkingAvailability == false && newDrug.id && (
                <p className="text-gray-800 italic text-sm">
                  Available stock: {inventoryDrug.locationQty}
                </p>
              )}
            </span>
          </div>
          {customerType == "hmo" && (
            <div className="md:col-span-2">
              <label
                htmlFor="customerItemName"
                className="block text-sm font-medium "
              >
                Customer Item
              </label>
              <SearchableDropdown
                value={inventoryDrug.item_id}
                isLoading={isLoading}
                data={
                  drugs
                    ? drugs.map((item) => ({
                ...
                onSelect={(value) => {
                  const selectedProcedure = drugs?.find(
                  ...
                  setNewDrug({
                    ...newDrug,
                    ...(!newDrug.quantity && { quantity: 1 }),
                    unit: selectedProcedure?.rate ?? newDrug.unit,
                    price:
                      (selectedProcedure?.rate ?? newDrug.unit) *
                      newDrug.quantity,
                  });
                }}
                placeholder="Select Providers procedure name"
                disabled={!!!form.values.customer || !!!newDrug.id}
              />
            </div>
          )}
          ...
          {form.values.drugs.map((drug, index) => (
            ...
            >
              <div className="grid grid-cols-12 gap-2 items-center">
                ...
                <div
                  className="col-span-2 text-center text-sm text-gray-600 truncate"
                  title={drug.price.toString()}
                >
                  ₦{drug.unit.toFixed(2)}
                </div>
